...
//captamos los datos POST enviados desde lista_estudiantes
  ...
  $query_carrera = "SELECT nombre, res_autorizacion, fecha_autorizacion, res_revalidacion, fecha_revalidacion FROM escuelaprofesional WHERE idEscuelaProfesional = '$carrera'";
  ...
  $a_valor_carrera=explode(" ", $valor_carrera[1]);
  $a_valor_carrera1=explode(" ", $valor_carrera[3]);
  $valor_carrera_cre_d=$a_valor_carrera[0];
  $valor_carrera_cre_n=$a_valor_carrera[1];
  $valor_carrera_rev_d=$a_valor_carrera1[0];
  $valor_carrera_rev_n=$a_valor_carrera1[1];
...
$objPHPExcel->setActiveSheetIndex(0)->setCellValue('B7','DATOS DEL INSTITUTO SUPERIOR TECNOLOGICO')
                                    ...
                                    ->setCellValue('S9',$valor_carrera_cre_d)//TIPO DE DOCUMENTO DE CREACION
                                    ->setCellValue('Q10','N°')
                                    ->setCellValue('S10',$valor_carrera_cre_n)//Numeracion de documento
                                    ->setCellValue('Q11','Fecha')
                                    ->setCellValue('S11',$valor_carrera[2])//FECHA DE EMISION
                                    ->setCellValue('M12','Resolucion de Revalidacion')
                                    ->setCellValue('Q12','Tipo')
                                    ->setCellValue('S12',$valor_carrera_rev_d)//TIPO DE DOCUMENTO DE REVALIDADCION
                                    ->setCellValue('Q13','N°')
                                    ->setCellValue('S13',$valor_carrera_rev_n)//NUMERACION DE COMENTO
                                    ->setCellValue('Q14','Fecha')
                                    ->setCellValue('S14',$valor_carrera[4])//FECHA DE EMISION
...
while ($lista_lista = mysqli_fetch_row($val_lista)) {
    ...
    $ud_est_posicion = 'K';
        while ($lista_ud_est = mysqli_fetch_row($var_ud_est)) {
          ...
          $query_verificacion = "SELECT idmatricula 
                                FROM matricula
          ...
          $lista_verificacion = mysqli_fetch_row($var_verificacion);
              if (!is_null($lista_verificacion[0])) {
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue($ud_est_posicion.$num_lista, "✓");//para cursos llevados
              }
              else{
...
$objPHPExcel->getActiveSheet()->getStyle('S9:S15')->applyFromArray($center_subtitle); 
